...
% \renewcommand{\flat}{\musFlat}
% \renewcommand{\sharp}{\musSharp}
      ...
      \begin{tikzpicture}[baseline=(staff.base),scale=0.50,every node/.style={inner sep=0pt,outer sep=0pt}]
        ...
        \ifnum\acc>0
          ...
          \foreach [count=\i] \pos in \sharpPos {%
            \ifnum\i>\countacc\relax\else
              ...
              \node at (\xpos,\pos*\staffStep-0.148cm) {$\musSharp$};
            \fi
          }
        \fi
        % flats
        \ifnum\acc<0
          ...
          \foreach [count=\i] \pos in \flatPos {%
            \ifnum\i>\countacc\relax\else
              \pgfmathsetmacro{\xpos}{0.25*(\i-1)+0.23}%
              \node at (\xpos,\pos*\staffStep-0.14cm) {$\musFlat$};
            \fi
          }
        \fi
      \end{tikzpicture}
...
\newcommand{\sectorNodeDefault}[4]{%
  ...
  \ifrotateKeySignatures
    \node[font=\bfseries\large, inner sep=-1.8pt, rotate=#2-90] (main) at (#2:#3+0.05) {\note};
    \node[font=\scriptsize, anchor=north, rotate=#2-90, text=white!30!black] at (main.south) {\saxnote};
  \else
  ...
  \ifrotateKeySignatures
    \node[anchor=base, rotate=#2-90, shift={(0,-\staffStep)}] at (#2:{\rotatedsheetdistance+#4}) {\drawKeySignature{\pc}};
  \else
...
\newcommand{\innerSectorNoteMinor}[4]{%
  ...
  \ifrotateKeySignatures
    \node[font=\bfseries\large, inner sep=-1.8pt, rotate=#2-90] at (#2:#3) {\small \note m};
  \else
...
\begin{document}
\begin{tikzpicture}
\drawCircularSectors{2.1}{1.0}{12}{\sectorNodeDefault}
\drawCircularSectors{1.5}{0.6}{12}{\innerSectorNoteMinor}
\end{tikzpicture}
\end{document}
